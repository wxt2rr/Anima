...
KEYS: List[ConfigKeySpec] = [
    ...
    ConfigKeySpec("coder", "timeout_ms", "settings.coder.timeoutMs", "int", "Coder 单次调用超时毫秒", "设置 -> Coder -> 超时"),
    ConfigKeySpec("coder", "max_output_chars", "settings.coder.maxOutputChars", "int", "Coder 最大输出字符数", "设置 -> Coder -> 最大输出"),
    ConfigKeySpec("coder", "message_mode", "settings.coder.resultPolicy.messageMode", "string", "Coder 消息返回模式", "设置 -> Coder -> 返回策略", choices=["all", "last", "summary"]),
    ConfigKeySpec("coder", "artifact_mode", "settings.coder.resultPolicy.artifactMode", "string", "Coder 附件返回模式", "设置 -> Coder -> 返回策略", choices=["none", "final", "all"]),
    ConfigKeySpec("coder", "include_decision_requests", "settings.coder.resultPolicy.includeDecisionRequests", "bool", "是否返回待决策消息", "设置 -> Coder -> 返回策略"),
    ConfigKeySpec("status_center", "tray_enabled", "settings.statusCenter.tray.enabled", "bool", "启用菜单栏状态图标", "设置 -> 状态中心 -> 启用菜单栏图标"),
    ConfigKeySpec("status_center", "tray_animated", "settings.statusCenter.tray.animated", "bool", "启用运行中动画", "设置 -> 状态中心 -> 启用运行中动画"),
    ConfigKeySpec("status_center", "frame_interval_ms", "settings.statusCenter.tray.frameIntervalMs", "int", "动画间隔毫秒", "设置 -> 状态中心 -> 动画间隔"),
    ConfigKeySpec("status_center", "fallback_to_builtin", "settings.statusCenter.tray.fallbackToBuiltin", "bool", "失败时回退内置图标", "设置 -> 状态中心 -> 回退策略"),
    ConfigKeySpec("status_center", "icon_idle_22", "settings.statusCenter.tray.icons.idle.sizes.22", "string", "空闲状态 22px 图标路径", "设置 -> 状态中心 -> 空闲图标"),
    ConfigKeySpec("status_center", "icon_running_22", "settings.statusCenter.tray.icons.running.sizes.22", "string", "运行中状态 22px 图标路径", "设置 -> 状态中心 -> 运行中图标"),
    ConfigKeySpec("status_center", "icon_waiting_22", "settings.statusCenter.tray.icons.waiting_user.sizes.22", "string", "等待用户状态 22px 图标路径", "设置 -> 状态中心 -> 等待用户图标"),
    ConfigKeySpec("status_center", "icon_done_22", "settings.statusCenter.tray.icons.done.sizes.22", "string", "完成状态 22px 图标路径", "设置 -> 状态中心 -> 完成图标"),
    ConfigKeySpec("status_center", "icon_error_22", "settings.statusCenter.tray.icons.error.sizes.22", "string", "错误状态 22px 图标路径", "设置 -> 状态中心 -> 错误图标"),
    ConfigKeySpec("status_center", "running_frames", "settings.statusCenter.tray.icons.running.frames", "json", "运行中动画帧路径列表", "设置 -> 状态中心 -> 运行中动画帧"),
    ...
    ConfigKeySpec("network", "proxy_mode", "settings.proxyMode", "string", "代理模式", "设置 -> 网络 -> 代理模式", choices=["auto", "manual"]),
    ConfigKeySpec("network", "proxy_url", "settings.proxyUrl", "string", "代理地址", "设置 -> 网络 -> 代理地址"),
